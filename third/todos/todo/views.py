...
# Доступ к классу User(готовая таблица пользователя)
from django.contrib.auth.models import User
# Ошибки при работе с бд
from django.db import IntegrityError
# автоматически авторизация при регистрации(вход в аккаунт); выход из аккаунта; авторизация
from django.contrib.auth import login, logout, authenticate
from .forms import TodoForm
from .models import Todo
from django.shortcuts import get_object_or_404
# модуль заполнения времени
    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                login(request, user)
                # Перевод пользователя на обозначенную страницу
                return redirect('currenttodos')
...
def currenttodos(request):
    # фильтрация только выполнены заданий по date_completed
